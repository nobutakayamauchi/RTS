...
Before a new program or deliberately constructed challenger may claim superiority, refresh the surrounding implementation landscape.
...
Materialize the strongest existing/external composition and the smallest justified custom/glue challenger. Attack them with the same frozen workload, evidence requirements, authority boundaries, destructive cases, recovery obligations, and whole-life criteria.
...
Every promoted implementation lives in a Movable Frame. A credible challenger caused by model capability, OS/platform change, API/OSS/SaaS change, price, provider behavior, security, user burden, law/procedure, runtime evidence, or a new failure class inherits the same workload and known death causes.
...
WATCH observes material changes in models/LLMs, libraries, APIs, algorithms, OSS, competitors, infrastructure, pricing, degradation/deprecation, vulnerabilities, shutdowns, and real-world evidence. A discovery is a candidate trigger, not automatic replacement authority.
