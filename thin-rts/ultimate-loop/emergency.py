...
class EmergencyGateError(ValueError):
    pass


def _time(value: Any, field: str) -> datetime:
    if not isinstance(value, str) or not value or value.strip() != value:
        raise EmergencyGateError(f"{field} must be an exact ISO-8601 timestamp")
    text = value[:-1] + "+00:00" if value.endswith("Z") else value
    try:
        parsed = datetime.fromisoformat(text)
    except ValueError as exc:
        raise EmergencyGateError(f"{field} must be ISO-8601") from exc
    if parsed.tzinfo is None:
        raise EmergencyGateError(f"{field} must include timezone")
...
def _candidate_preflight(case: dict[str, Any], at: datetime) -> list[str]:
    ...
    if mode not in {"STATELESS", "READ_ONLY", "READ_WRITE_SINGLE_WRITER"}:
        raise EmergencyGateError("unsupported operation_mode")
...
def _applied_recovery(
    ...
    if any(not _text(recovery.get(key)) for key in required_text):
        raise EmergencyGateError("applied recovery requires bound trigger/executor/authority evidence")
    if recovery.get("candidate_id") != candidate["candidate_id"]:
        return {"state": "RECOVERY_NOT_VALIDATED", "action": "RETURN_TO_ANALYSIS", "temporary_occupant": True, "promotion_authorized": False}
    if recovery.get("trigger_state") not in {"FAILED", "UNSAFE"}:
        raise EmergencyGateError("applied recovery requires FAILED or UNSAFE trigger snapshot")
    ...
    if not (trigger_at < applied_at <= at):
        raise EmergencyGateError("recovery application time must follow its persisted trigger observation and not be future")
...
def evaluate(
    ...
    *,
    ...
    """Evaluate the bounded Emergency Recovery / Failover evidence contract.

    The gate owns no monitoring, traffic switching, restart, provider API, secret,
    scheduler, autonomous failover, or promotion authority.
    """
    if not isinstance(case, dict) or not isinstance(case.get("emergency"), dict):
        raise EmergencyGateError("emergency case required")
    emergency = case["emergency"]
    policy = emergency.get("policy")
    health = emergency.get("health")
    if not isinstance(policy, dict) or not isinstance(health, dict):
        raise EmergencyGateError("emergency policy/health required")
    state = health.get("state")
    if state not in {"HEALTHY", "DEGRADED", "FAILED", "UNSAFE", "UNKNOWN"}:
        raise EmergencyGateError("invalid health state")
    if not _text(health.get("source_ref")) or not _text(health.get("observed_at")) or not _text(health.get("stale_after")):
        raise EmergencyGateError("current health evidence required")
    ...
    if health_at > at:
        raise EmergencyGateError("health observation cannot be in the future")
