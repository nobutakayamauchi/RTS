...
CHECKPOINT_FIELDS = {
    "schema_version",
    "checkpoint_id",
    "source_campaign_close_checkpoint_fingerprint",
...
def verify_template_contract(contract: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    authorization = value.get("design_authorization")
    if authorization != {
        "type": "HUMAN",
    ...
    if value.get("designer") != {
        "type": "AI_ASSISTANT",
        "role": "REPORT_TEMPLATE_DESIGNER",
...
def build_template(contract: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    template = {
        ...
        "section_contract": {
            section: {
                "required": True,
                "human_review_required": section == "human_review_decision",
            }
...
def _round_source(run: dict[str, Any], round_id: str) -> dict[str, Any]:
    value = next((item for item in run["rounds"] if item["round_id"] == round_id), None)
    if value is None:
        raise ProofEngineError(f"missing report source round: {round_id}")
    return value


def _effective_records(source_round: dict[str, Any], review: dict[str, Any]) -> list[dict[str, Any]]:
    originals = {item["candidate_id"]: item for item in source_round["candidates"]}
    ...
    for effective in review["effective_candidates"]:
        ...
        if candidate_fingerprint != effective["candidate_fingerprint"]:
...
def _build_repository_report(
    *,
    ...
    records = _effective_records(source_round, review)
    merged_prs = [
        {"number": item["number"], "title": item["title"], "status": item["status"]}
        for item in source_round["selected_prs"]
        if item.get("merged") is True and item.get("status") == "MERGED"
    ]
    excluded_prs = [item["number"] for item in source_round["selected_prs"] if item.get("merged") is not True]
    withheld = [
        {
            "claim": item["claim"],
            "reason": item["reason"],
        ...
        for item in source_round.get("withheld_claims", [])
    ]
    repository = source_round["repository"]
    sections = {
        "executive_summary": {
            ...
            "achievement_count": len(records),
            "withheld_claim_count": len(withheld),
        },
        "repository_scope": {
            "repository": repository,
            "visibility": source_round["visibility"],
            "source_mode": source_round["source_mode"],
            "snapshot_ref": source_round["snapshot_ref"],
            "role": source_round["role"],
        },
        "methodology": {
            "steps": [
        ...
        "evidence_inventory": {
            "readme_blob_sha": source_round["readme_blob_sha"],
            "merged_prs": merged_prs,
            "excluded_unmerged_prs": excluded_prs,
            "source_round_fingerprint": source_round["round_fingerprint"],
        },
...
def build_demonstration_pack(contract: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    run = close["source_bundle"]["run"]
    round2 = verify_round_two_review_bundle()["review"]
    round3 = close["previous"]["review"]
    round4 = close["review"]
    reviews = {"ROUND-2": round2, "ROUND-3": round3, "ROUND-4": round4}
    reports = [
        _build_repository_report(
            round_id=round_id,
            source_round=_round_source(run, round_id),
...
def render_demonstration_markdown(bundle: dict[str, Any] | None = None) -> str:
    ...
    for report in value["pack"]["reports"]:
        ...
        if withheld:
            ...
            lines.append("- None in the selected evidence boundary.")
