...
- ESC evidence snapshot is OPTIONAL and only happens on breakpoint.
...
- 0: OK
- 2: usage error
- 3: DEGRADED (ledger integrity issues detected; output still written)
- others: unexpected failure

Usage:
  python scripts/session_rollup.py 2026-03
  python scripts/session_rollup.py 2026-03 20260301
"""
...
def compute_transitions_and_metrics(
    ...
    # recovery_speed_runs: from last failure to next success
...
def main() -> int:
    ...
    # Evidence: prefer evidence_index.json, fallback to directory scan
    ...
    # If ledger issues exist, still write outputs but signal DEGRADED for pipelines
    if issues:
        return 3
    return 0
