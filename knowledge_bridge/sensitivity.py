...
_PERSONAL_PATTERNS = (
    ...
    ("medical", re.compile(r"(?i)\b(?:diagnosis|medical|hospital|medication|診断|病院|服薬|症状)\b")),
    ("employment_dispute", re.compile(r"(?i)\b(?:harassment|dismissal|labor dispute|パワハラ|退職勧奨|労働問題)\b")),
    ("financial", re.compile(r"(?i)\b(?:bank account|credit card|debt|口座|借金|カード番号|金銭難)\b")),
)
...
def assess_sensitivity(text: str, frontmatter: dict[str, Any] | None = None) -> SensitivityResult:
    ...
    if explicit in {"public", "internal", "personal", "restricted"}:
        level = explicit
        reasons = ("explicit_frontmatter",)
        return SensitivityResult(level, reasons, level == "public")
