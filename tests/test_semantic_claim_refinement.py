...
class SemanticClaimRefinementTests(unittest.TestCase):
    def test_unique_semantic_alias_reduces_review(self):
        anchor = "A request may dispatch several functions concurrently."
        intake = make_intake(anchor)
        ...
        self.assertEqual(semantic[0]["anchor"], anchor)
    ...
    def test_unmatched_contract_text_stays_review_required(self):
        intake = make_intake("A request now uses a new runtime policy for advanced workflows.")
        refined = refine_intake_report(intake)
    ...
    def test_upstream_truncation_blocks_ready_even_when_listed_anchor_resolves(self):
        intake = make_intake("A request may dispatch several functions concurrently.")
    ...
    def test_input_fingerprint_prevents_stale_verification(self):
        intake = make_intake("A request may dispatch several functions concurrently.")
        refined = refine_intake_report(intake)
        changed = make_intake("A request now uses a new runtime policy for advanced workflows.")
